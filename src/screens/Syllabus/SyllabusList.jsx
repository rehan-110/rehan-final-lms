...
export default function SyllabusList() {
  ...
  useEffect(() => {
    return onSnapshot(collection(db, 'syllabus'), (snap) =>
      setRows(
        snap.docs.map((d) => ({
          id: d.id,
          ...d.data(),
  ...
  return (
    <Box p={2}>
      ...
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
        ...
        />
        <Button variant="contained" onClick={() => navigate('/syllabus/add')}>
          Add New Syllabus
        </Button>
      </Box>
              ...
              .map((row) => (
                <TableRow key={row.id} selected={isSelected(row.id)}>
                  ...
                  <TableCell align="center">
                    <IconButton
                      color="primary"
                      onClick={() => navigate(`/syllabus/edit/${row.id}`)}
                    >
                      <EditIcon />
                    </IconButton>
                    <IconButton
                      ...
                      onClick={async () => {
                        if (window.confirm('Delete this syllabus?')) {
                          await deleteDoc(doc(db, 'syllabus', row.id));
                        }
                      }}
                    >
                      <DeleteIcon />
